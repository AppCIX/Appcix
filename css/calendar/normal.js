...
// ---- Common Options ----
var gMonths=["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Set","Oct","Nov","Dic"];
...
var gBegin=[ano-60,mes+1,diames+1];
var gEnd=[ano+1,mes+1,diames+1];	// calendar date range end at [Year,Month,Date]
var gsOutOfRange="Lo Siento, esta eligiendo una Fecha fuera del parametro permitido!";	// out-of-date-range error message. If set to "", no alerts will popup on such error.
...
var gbHideBottom=false;	// true: hide the bottom section; false: show it with gsBottom.
	var gsBottom="<A href='javascript:void(0)' class='BottomAnchor' onclick='if(this.blur)this.blur();if(!fSetDate(gToday[0],gToday[1],gToday[2]))alert(\"No se puede seleccionar la Fecha Actual!\");return false;' onmouseover='return true;' >Hoy es : "+gMonths[gToday[1]-1]+" "+gToday[2]+", "+gToday[0]+"</A>";	// the content of the bottom section.
...
var giWeekTop=1;	// calendar week-number-column top offset;
